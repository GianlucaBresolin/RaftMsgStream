...
func TestUnvotingServerLogic(t *testing.T) {
	...
	node1 := NewRaftNode("node1", ":5001", server1, map[ServerID]Address{"node2": ":5002", "node3": ":5003"}, false)
	server2 := rpc.NewServer()
	node2 := NewRaftNode("node2", ":5002", server2, map[ServerID]Address{"node1": ":5001", "node3": ":5003"}, false)
	server3 := rpc.NewServer()
	node3 := NewRaftNode("node3", ":5003", server3, map[ServerID]Address{"node1": ":5001", "node2": ":5002"}, false)

	server4 := rpc.NewServer()
	unvotingNode := NewRaftNode("node4", ":5005", server4, map[ServerID]Address{"node1": ":5001", "node2": ":5002", "node3": ":5003"}, true)
